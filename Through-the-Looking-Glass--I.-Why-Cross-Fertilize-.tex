...
\hypersetup{
  ...
  pdfkeywords={evolution, coevolution, host-parasite
interactions, Muller's ratchet, parthenogenesis, Red Queen
...
\href{mailto:clively@iu.edu}{email}.
...
Lively, Curtis M. 2024. \emph{Through the Looking Glass: I. Why
Cross-Fertilize?} Indiana University Bloomington Libraries Publishing.
...
\begin{center}
\includegraphics[width=0.4\linewidth,height=\textheight,keepaspectratio]{images/fig1-1.jpeg}
\end{center}
...
reasoning, but I hope to at least convince them that there is a crisis
at hand in evolutionary biology.\ldots{}''
...
was doing field work in Sonora, Mexico, sometimes under very harsh field
...
was no clear general explanation for sex. Many years later, I am still
...
their self-fertilized or parthenogenetic offspring.\footnote{It is not
  ...
  resources in outcrossed kin with a relatively low level of relatedness
  (\(r = 0.5\)), rather than in self-fertilized kin with a high level of
  relatedness (\(r = 1\))? (See \citeproc{ref-dagg2016a}{Dagg 2016}.)}
(See \hyperref[sec-glossary]{Glossary} for condensed definitions.)
Williams' idea also had theoretical support, as R.A. Fisher
...
\includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio,alt={Diagram illustrating a sexual female producing one daughter and one son and an asexual female producing two daughters.}]{images/fig1-2.jpg}
...
is an algebraic solution, which suggests that (1) the two costs are
mutually exclusive, and (2) they apply to different kinds of uniparental
progeny (\citeproc{ref-lively1990b}{Lively \& Lloyd 1990}). Roughly
speaking, I think we can adopt the following rules for the purpose of
this book. When considering the spread of a rare allele that induces
self-fertilization in hermaphrodites, the appropriate cost is Williams'
...
producing males. The two costs do not combine. The cost of sex is not
four-fold.
...
\includegraphics[width=0.55\linewidth,height=\textheight,keepaspectratio,alt={Two data visualizations of simulation results that show the replacement of sexual individuals by a parthenogenetic clone in tens of generations.}]{images/fig1-3_hr.png}
...
visualization} can be explored using an internet browser. In addition,
...
females, or between outcrossing and selfing, but rather between alleles
that modify the rate of recombination. So instead of asking ``Why
cross-fertilize?'' we can assume cross-fertilization and ask, ``Why is
...
meiosis. Indeed, Lewontin (\citeproc{ref-lewontin1971a}{1971}) formally
showed that ``the mean fitness of the population at equilibrium is a
maximum in the absence of recombination.''\footnote{Lewontin
...
Even before the costs of meiosis and males were so dramatically revealed
by Williams and Maynard Smith, biologists were reckoning with the
anomaly of sex (reviews in \citeproc{ref-dagg2016a}{Dagg 2016};
...
on three species of a curious plant in the genus \emph{Primula}. The
plant is curious in that it has two morphs. One morph has a style that
extends beyond the anthers (the long-style morph), and the other morph
has anthers that extend beyond the style (the short-style morph).
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Drawing showing the short-style and long-style forms of *Primula* used in Darwin\textquotesingle s pollination experiment.}]{images/fig1-4.jpg}}
...
theory.\footnote{For example, Darwin calls the evolution of sterile
  ...
  \citeproc{ref-darwin1859a}{Darwin 1859}).}
...
\caption{\label{tbl-t1t2}Appendix: Maynard Smith's Model}
...
\begin{center}
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{images/fig2-1.jpeg}
\end{center}
...
variable polyculture (Table~\ref{tbl-bh1} \& Table~\ref{tbl-bh2}). Let's
assume that the variation in yield is driven by annual variation in
abiotic factors such as temperature or precipitation. The effect of
...
polyculture.}\label{tbl-bh1}\tabularnewline
\toprule\noalign{}
...
\caption{\label{tbl-bh2}Bet Hedging. Summary statistics for the data in
Table~\ref{tbl-bh1}.}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Arithmetic mean} & 240 & 220 \\
\textbf{Variance}{\footnote{The variance in \(x\) is equal to the
...
assume that the niche width of a single clone would be narrow relative
to the niche width of the genetically diverse sexual population. So,
under this idea, a clone could invade a sexual population and perhaps
displace it from one of its many niches. But a single clone could not
completely replace the sexual population
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={Diagram of four possibilities derived from crossing density-dependent and density-independent selection with frequency-dependent and frequency-independent selection.}]{images/fig2-2_hr.png}
...
infect the most-common host genotype, the reproductive advantage of a
common host clone is eroded. Moreover, if the parasite is \emph{common
and sufficiently virulent}, evolution by the parasite could prevent the
...
\includegraphics[width=0.65\linewidth,height=\textheight,keepaspectratio,alt={Two data visualizations of simulation results that show that as the asexuals become common, the parasite evolves to disproportionately infect them, driving their number down.}]{images/fig2-3_hr.png}
...
\citeproc{ref-peters2007a}{2007}; \citeproc{ref-salathe2008a}{Salathé
...
\includegraphics[width=0.65\linewidth,height=\textheight,keepaspectratio,alt={Data visualization of simulation results showing oscillations in host genotype and parasite genotype frequencies. The parasite\textquotesingle s oscillations lag 90 degrees behind the host.}]{images/fig2-4_hr.png}
...
biological hostility of the environment.''
...
{\centering \includegraphics[width=0.65\linewidth,height=\textheight,keepaspectratio,alt={Data visualization showing the change in \$q (\textbackslash Delta q)\$ plotted against \$q\$. There is an equilibrium for \$q\$ at \$\textbackslash Delta q=0\$.}]{images/fig2-5_hr.png}
...
\begin{center}
\includegraphics[width=0.33\linewidth,height=\textheight,keepaspectratio]{images/fig3-1.jpeg}
\end{center}
...
(\citeproc{ref-lively1986c}{Lively 1986c}).\footnote{Plasticity is more
  ...
  determined, not the morph per se (see also
  \citeproc{ref-lively2000a}{Lively \emph{et al.} 2000};
  \citeproc{ref-hazel2004a}{Hazel \emph{et al.} 2004}).} Hence, the two
morphs are not different species, but rather the result of phenotypic
...
(\citeproc{ref-lively1986b}{Lively 1986b}). As the tide receded, the
snails moved out from these crevices onto the exposed rock surfaces to
forage on barnacles. When the tide returned, the snails motored back to
the crevices, presumably to hide from snail-crushing rays that came in
with the tide. This back-and-forth movement of snails created
...
(\citeproc{ref-lively1986b}{Lively 1986b}). Hence there is a trade-off.
Taken together, the results suggested that plastic development was
favored by natural selection to survive in the high-predation zones
(Section~\ref{sec-app-3}). I would later come to think of adaptive
plasticity as a type of variation strategy. Sexual reproduction can also
be seen as a type of variation strategy (\citeproc{ref-lloyd1984a}{Lloyd
1984}). And I was very fortunate to be able to study sexual reproduction
...
\includegraphics[width=0.3\linewidth,height=\textheight,keepaspectratio,alt={Drawings of the two shell morphs of the intertidal barnacle, *Chthamalus anisopoma*.}]{images/fig3-2.jpeg}
...
\includegraphics[width=0.3\linewidth,height=\textheight,keepaspectratio,alt={Drawing of the predatory snail, *Acanthina angelica*, attacking the bent form of the barnacle.}]{images/fig3-3.jpeg}
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={Photo of the freshwater New Zealand snail, *Potamopyrgus antipodarum*.}]{images/fig3-4.jpeg}
...
(\citeproc{ref-may1983a}{1983}). They showed in a theoretical paper that
...
biologists in the history of evolutionary thought. He was able to
formulate and communicate novel ideas with apparent ease. Around the
time that I was beginning to work on \emph{Potamopyrgus}, JMS came to
New Zealand, along with his wife, Sheila. He was invited by David Lloyd
to spend time at the University of Canterbury and to deliver three
public lectures, which were all fantastic. During this time, JMS spent
several weeks in New Zealand. Lynda, David, and I were lucky enough to
hang out with him quite a bit. JMS was a remarkable individual. He could
talk with anyone and show a sincere interest in their work. One morning,
I was sitting next to JMS in the tearoom in the old Zoology Department.
I was scared speechless. He kindly asked me what I was working on, so I
told him about the snails. He knew of them! In fact, he had covered them
in his book, \emph{The Evolution of Sex}.\footnote{With respect to
...
parasites might be a factor in selecting for cross-fertilization and
recombination in hosts. But my study as well as the study of Burt and
Bell (\citeproc{ref-burt1987a}{1987}) were based on correlations. And
every scientist knows that correlation is not causation. On the other
hand, these correlations were predicted \emph{a priori} by Lloyd
...
specifically to mathematical models, the same principle applies to data
from biological systems. Ideally, the multiple lines of evidence would
include long-term field observations of individual populations, broader
biogeographic patterns across populations, and direct experiments using
multiple independent systems as well as multiple theoretical forays into
the conditions under which the hypothesis is expected to hold.
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={Scatter plot showing the percentage of males in a population against the prevalence of infection in females in the same population.}]{images/fig3-5_hr.png}
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={A map of New Zealand with pie charts showing the percentage of male and female *Potamopyrgus antipodarum* in New Zealand\textquotesingle s lakes.}]{images/fig3-6_hr.png}
...
is consistent with David Lloyd's (\citeproc{ref-lloyd1980a}{1980})
...
parasites are only periodically common. For this reason, Jukka
Jokela\footnote{I will write much more about Jukka's many contributions
  in the next volume.} and I selected 20 of the best sampled lakes from
the data set given in Figure~\ref{fig-3-5}. We then resampled all 20
lakes 10 -- 15 years after my original samples (with greater coverage of
the lakes and larger sample sizes). We found that prevalence of
infection within lakes was highly correlated between sample periods.
This result suggests that lakes for which parasite-mediated selection
was relatively high during the first set of samples was also relatively
high in the second set of samples. In addition, male frequency within
lakes was also highly correlated between sample periods
(\citeproc{ref-lively2002a}{Lively \& Jokela 2002}). We then averaged
the data for each lake across time periods under the assumption that the
averages would better represent the frequency of males and the
prevalence of infection over time. With these averages, the correlation
between male frequency and infection prevalence was positive and
statistically significant.\footnote{\(r = 0.47\); \(P = 0.04\) for
...
\item
  canalized development into a low-fecundity, predation-resistant morph,
\item
...
\pandocbounded{\includegraphics[keepaspectratio]{images/fig3-7_hr.png}}
...
\begin{center}
\includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{images/fig4-1.jpeg}
\end{center}
...
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio,alt={Illustration of the life cycle of the trematode *Microphallus*.}]{images/fig4-2.jpeg}
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={Photo of an infected and an uninfected snail (with their shells removed).}]{images/fig4-3.jpg}
...
\includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio,alt={Data visualization showing the results of the first reciprocal cross-infection experiment, which suggest strong parasite local adaption.}]{images/fig4-4.png}
...
very robust in experiments conducted by my students after I moved to
Indiana University (Figure~\ref{fig-4.5}).

\begin{figure}

\centering{

\includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio,alt={Data visualization showing the results of the second reciprocal cross-infection experiment using host and parasites from three different lakes. These results also show strong local adaptation.}]{images/fig4-5.png}
...
world with an experiment showing that oscillations in Red Grouse
densities were controlled by nematode infections. Parasites were now on
the map. An emerging new field called ``ecology and evolution of
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={Data visualization showing the results of a meta-analysis across 32 comparisons showing very general parasite local adaptation.}]{images/fig4-6.png}
...
does not seem at all unwise. Importantly, he suggests that infection
...
\includegraphics[width=0.55\linewidth,height=\textheight,keepaspectratio,alt={A diagram of self- / non-self-recognition in which the top two drawings show fusion of tunicate colonies that share one or more alleles at a recognition locus, while the bottom drawing shows that colonies do not fuse if they have no alleles in common.}]{images/fig4-7.jpeg}
...
\begin{center}
\pandocbounded{\includegraphics[keepaspectratio]{images/fig5-1.jpeg}}
\end{center}
...
if the sexual population contained more genetic variation for resistance
to infection. So, after correcting for size, the sexual fish were both
less infected and showed more variation in infection than the clone
2.\footnote{In a follow-up study (using different samples), Steve Weeks
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={Two scatter plots showing greater infection in clone 1 than in sexual fish in two different years.}]{images/fig5-2.png}
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={Scatter plot showing greater infection in the more common clone (clone 2) than in the rarer clone (clone 1).}]{images/fig5-3.png}
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={Two scatter plots showing that inbreed sexual fish were more infected than clone 1, but outbred sexual fish were less infected than clone 1.}]{images/fig5-4.png}
...
\begin{center}
\includegraphics[width=0.35\linewidth,height=\textheight,keepaspectratio]{images/fig6-1.jpeg}
\end{center}
...
to have the electricity turned on before our arrival. Luckily, we still
had our down sleeping bags, which we had purchased for field work in the
...
pointed out by May and Anderson (\citeproc{ref-may1983a}{1983}). This is
the same study that launched a massive wave of studies on the evolution
of parasite virulence, but it is also one of the first theoretical
studies on the Red Queen Hypothesis for sex. May and Anderson concluded
that the Red Queen could work, but only if parasites killed their hosts
(i.e., they were maximally virulent).\footnote{May and Anderson, for
  example, state, ``Our studies, in which the epidemiological details of
  the parasite--host interactions are treated more explicitly than in
  Hamilton's work, make this {[}the parasite{]} answer to the problem of
  sex (\citeproc{ref-ghiselin1974a}{Ghiselin 1974};
...
as we will see in Volume 2 (or see \citeproc{ref-lively2018a}{Lively
2018}).
...
introductory paper for a new journal. The editor graciously acknowledged
Muller in the preface to this first issue
...
population, including whether it became infected and whether it produced
offspring with one or more new mutations. Steve is a locked-on
...
to show that his model would converge on analytical solutions for the
...
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio,alt={Two data visualizations of fitness as a function of the number of mutations assuming independent effects. The top figure shows mean fitness at mutation-selection balance. The bottom figure shows the distribution of mutations at mutation-selection balance.}]{images/fig6-2.png}
...
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio,alt={Data visualization showing the distribution of mutations at time zero and 400 generations later. The greater number of mutations at generation 400 is due to mutation accumulation via Muller\textquotesingle s Ratchet.}]{images/fig6-3.png}
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={Data visualization showing the number of asexuals over time under three conditions: (1) parasites, but no mutation, (2) mutation, but no parasites, and (3) both parasites and mutation.}]{images/fig6-4.png}
...
Importantly, every time the clone was driven to a low frequency by
parasites, the dip was lower than in previous years. Hence, the number
of individuals in the LLC decreased, thereby allowing for a more rapid
operation of the ratchet. The clone is toast. Eventually the clone
undergoes a mutational meltdown and goes extinct
(\citeproc{ref-lynch1990a}{Lynch \& Gabriel 1990}).\footnote{Mutational
...
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio,alt={A plot showing the probability of parasite transmission against parasite virulence when there is no mutation. Sexual reproduction only wins in competition with both transmission and virulence is very high.}]{images/fig6-5.png}
...
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio,alt={Four plots like the one in Fig. 6.4, except that mutations are allowed to accumulate. The general result is that sexual reproduction wins for lower levels of virulence and transmission than the case without mutation.}]{images/fig6-6.png}
...
hypotheses to make different predictions before getting the data. What
if the answer is some combination or even synergism between hypotheses?
It could be that forcing predictions could result in the rejection of
all alternatives, when in fact two or more hypotheses were partially
true. And if two hypotheses are strongly synergistic, would it be
appropriate to claim that one was more important? In studying this
...
synergistically to select for sex in either parasites or hosts in the
wild.\footnote{Professor Hamilton told me at a meeting in London that he
  thought combining the ratchet and the Red Queen made sense, but that
  he did not think the ratchet was a necessary addition. See also the
  dissertation work by Olivia Judson for a detailed theoretical study of
  coexistence between sexuals and asexuals in a metapopulation
...
\begin{center}
\includegraphics[width=0.37\linewidth,height=\textheight,keepaspectratio]{images/fig7-1.png}
\end{center}
...
\tightlist
\item
  Do you think that asexual populations could have higher carrying
  capacities than sexual populations, as suggested in
  Figure~\ref{fig-1.2}? If so, under what conditions? If not, why not?
\item
...
The \emph{{D}aphnia magna-{P}asteuria ramosa} model}. \emph{PloS
Genetics}, 13.
...
Howard, R.S. \& Lively, C.M. (2002). The ratchet and the {Red Queen}:
The maintenance of sex in parasites. \emph{Journal of Evolutionary
Biology}, 15, 648--656.
...
Salathé, M., Kouyos, R.D., Regoes, R.R. \& Bonhoeffer, S. (2008). Rapid
...
\chapter*{Errata and Corrections}\label{errata-and-corrections}
\addcontentsline{toc}{chapter}{Errata and Corrections}

\markboth{Errata and Corrections}{Errata and Corrections}

This page is maintained by IU Libraries Publishing and documents
post-publication corrections to this edition.

\section*{2026-07-17}\label{section}
\addcontentsline{toc}{section}{2026-07-17}

\markright{2026-07-17}

In Figure~\ref{fig-6-2}, the mean fitness value for time 400, shown in
red on the right, was corrected from \(\bar{w} = 0.82\) to
\(\bar{w} = 0.75\).

This correction does not affect the accompanying analysis,
