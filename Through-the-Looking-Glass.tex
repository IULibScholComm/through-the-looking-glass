...
offspring\sidenote{\footnotesize It is not meiosis per se that is costly. As Williams
...
value\sidenote{\footnotesize Asexuals are often polyploid versions of their sexual
...
absence of recombination}\sidenote{\footnotesize Lewontin (1971) was following up on
...
Darwin was drawn to anomalies on theory\sidenote{\footnotesize For example, Darwin
...
attention\sidenote{\footnotesize Darwin (1860): \emph{There is no greater mystery in
...
discovery of parthenogenesis\sidenote{\footnotesize The case could has been made that
...
wonder is that it should not oftener occur}\sidenote{\footnotesize I suspect that
...
\hypertarget{box-1.2}{%
\section{Box 1.2}\label{box-1.2}}

Maynard Smith's (1978) model showing the cost of producing
males.\sidenote{\footnotesize I used slightly different variable names, and I tried to
  simplify JMS's original model.} Let \(N~asex~\) be the number of
asexual females at time 1, while \(N~sex~\) gives the total number of
sexual individuals (males plus females) at time 1. Let \(B~asex~\) give
the number of offspring produced by asexual females, and \(S~asex~\)
gives the survival probability of asexual offspring to maturity. The
number of surviving asexual offspring is then \(= B~asex~S~asex~\).
Similarly, let \(B~sex~\) be the number offspring produced by sexual
females, and let \(S~sex~\) give the survival probability of sexually
produced offspring. Maynard Smith assumed that all individuals reproduce
once and then die. Let \(r\) be the frequency of females in the sexual
