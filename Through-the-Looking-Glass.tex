...
female in a sexual population at carrying capacity, \(K~sex~\). At
\(K~sex~\), the sexual females are, on average, producing one daughter
...
\CommentTok{\# Simulation for competition between coexisting sexuals and asexuals, following}
\CommentTok{\# Lively 2009.  the simulation assumes that carrying capacity is set by the}
\CommentTok{\# parameters below, rather than being fixed.}

\NormalTok{a }\OtherTok{=} \FloatTok{1e{-}04}  \CommentTok{\#a is a constant that gives the sensitivity to total population density}
\NormalTok{d }\OtherTok{=} \DecValTok{1}  \CommentTok{\# d is the death rate.  Here I set d=1, meaning an annual species.}
\NormalTok{b }\OtherTok{=} \DecValTok{3}  \CommentTok{\# b is the number of offspring prouduced by a single female (sexual or asexual)}
\NormalTok{c }\OtherTok{=} \DecValTok{0}  \CommentTok{\# c is a constant that gives the sensitivity of the death rate to density.  Here set = 0.}
\NormalTok{s }\OtherTok{=} \DecValTok{1}\SpecialCharTok{/}\DecValTok{2}  \CommentTok{\# s is the frequency of males in the sexual subpopulation.  (1{-}s) gives the freq of females in sexual popl}


\CommentTok{\# anayltical solutions: carrying capacities for sexual and asexuals are set by}
\CommentTok{\# the parameters given above.  following Lively (2009) J Evol Biol. doi:}
\CommentTok{\# 10.1111/j.1420{-}9101.2009.01824.x}
\NormalTok{Ksex }\OtherTok{=}\NormalTok{ ((}\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ s) }\SpecialCharTok{*}\NormalTok{ b }\SpecialCharTok{{-}}\NormalTok{ d)}\SpecialCharTok{/}\NormalTok{((}\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ s) }\SpecialCharTok{*}\NormalTok{ a }\SpecialCharTok{+}\NormalTok{ c)  }\CommentTok{\#solution for carrying capacity of sexual population}
\NormalTok{Kasex }\OtherTok{=}\NormalTok{ (b }\SpecialCharTok{{-}}\NormalTok{ d)}\SpecialCharTok{/}\NormalTok{(a }\SpecialCharTok{+}\NormalTok{ c)  }\CommentTok{\#solution for carrying capacity of asexual population}

\CommentTok{\# intitial conditions.  Sex initiated at Ksex.  Asex at 0.}
\NormalTok{Sex }\OtherTok{=}\NormalTok{ Ksex}
\NormalTok{Asex }\OtherTok{=} \DecValTok{0}

\NormalTok{Gasex }\OtherTok{=} \DecValTok{40}  \CommentTok{\#generation at which a single asexual female is introduced}

\NormalTok{T }\OtherTok{=} \DecValTok{100}  \CommentTok{\# T is the number of time steps in addition to time step 0}
\NormalTok{time }\OtherTok{=} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\NormalTok{T)  }\CommentTok{\#sets up do loop for i = 1 to T}

\NormalTok{outSex }\OtherTok{=} \FunctionTok{vector}\NormalTok{()  }\CommentTok{\# outSex vector}
\NormalTok{outSex[}\DecValTok{1}\NormalTok{] }\OtherTok{=}\NormalTok{ Sex[}\DecValTok{1}\NormalTok{]}

\NormalTok{outAsex }\OtherTok{=} \FunctionTok{vector}\NormalTok{()  }\CommentTok{\# outAsex  vector}
\NormalTok{outAsex[}\DecValTok{1}\NormalTok{] }\OtherTok{=}\NormalTok{ Asex[}\DecValTok{1}\NormalTok{]}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\NormalTok{T)  }\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\NormalTok{T)}
\NormalTok{\{}
\NormalTok{    outSex[i] }\OtherTok{=}\NormalTok{ Sex[i]}
\NormalTok{    Sex[i }\SpecialCharTok{+} \DecValTok{1}\NormalTok{] }\OtherTok{=}\NormalTok{ Sex[i] }\SpecialCharTok{{-}}\NormalTok{ Sex[i] }\SpecialCharTok{*}\NormalTok{ (d }\SpecialCharTok{+}\NormalTok{ c }\SpecialCharTok{*}\NormalTok{ (Sex[i] }\SpecialCharTok{+}\NormalTok{ Asex[i])) }\SpecialCharTok{+}\NormalTok{ Sex[i] }\SpecialCharTok{*}\NormalTok{ (}\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ s) }\SpecialCharTok{*}
\NormalTok{        (b }\SpecialCharTok{{-}}\NormalTok{ a }\SpecialCharTok{*}\NormalTok{ (Sex[i] }\SpecialCharTok{+}\NormalTok{ Asex[i]))}

    \ControlFlowTok{if}\NormalTok{ (i }\SpecialCharTok{==}\NormalTok{ Gasex) \{}
\NormalTok{        Asex[i] }\OtherTok{=}\NormalTok{ Asex[i] }\SpecialCharTok{+} \DecValTok{1}
\NormalTok{    \}}

\NormalTok{    outAsex[i] }\OtherTok{=}\NormalTok{ Asex[i]}
\NormalTok{    Asex[i }\SpecialCharTok{+} \DecValTok{1}\NormalTok{] }\OtherTok{=}\NormalTok{ Asex[i] }\SpecialCharTok{{-}}\NormalTok{ Asex[i] }\SpecialCharTok{*}\NormalTok{ d }\SpecialCharTok{+}\NormalTok{ Asex[i] }\SpecialCharTok{*}\NormalTok{ (b }\SpecialCharTok{{-}}\NormalTok{ a }\SpecialCharTok{*}\NormalTok{ (Sex[i] }\SpecialCharTok{+}\NormalTok{ Asex[i]))}

\NormalTok{\}}

\NormalTok{Ksex}
\NormalTok{Kasex}
\CommentTok{\# Sex outSex}



\FunctionTok{plot}\NormalTok{(time, outAsex, }\AttributeTok{type =} \StringTok{"l"}\NormalTok{, }\AttributeTok{col =} \StringTok{"blue"}\NormalTok{, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{20000}\NormalTok{), }\AttributeTok{xlab =} \StringTok{""}\NormalTok{, }\AttributeTok{ylab =} \StringTok{""}\NormalTok{)}
\FunctionTok{par}\NormalTok{(}\AttributeTok{new =} \ConstantTok{TRUE}\NormalTok{)}
\FunctionTok{plot}\NormalTok{(time, outSex, }\AttributeTok{type =} \StringTok{"l"}\NormalTok{, }\AttributeTok{col =} \StringTok{"red"}\NormalTok{, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{20000}\NormalTok{), }\AttributeTok{xlab =} \StringTok{"Generation"}\NormalTok{,}
    \AttributeTok{ylab =} \StringTok{"Number"}\NormalTok{)}
\FunctionTok{legend}\NormalTok{(}\DecValTok{70}\NormalTok{, }\DecValTok{17500}\NormalTok{, }\AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"Asexuals"}\NormalTok{, }\StringTok{"Sexuals"}\NormalTok{), }\AttributeTok{col =} \FunctionTok{c}\NormalTok{(}\StringTok{"blue"}\NormalTok{, }\StringTok{"red"}\NormalTok{), }\AttributeTok{lty =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
carrying capacity: \(K~sex~ = 10000\). Note that the asexual lineage
replaces the sexual population in about 25 generations. The asexual
population then attains a new carrying capacity at \(K~asex~ = 20000\)
...
normal meiosis. Indeed, Lewontin (1971) formally showed that: \emph{...
the mean fitness of the population at equilibrium is a maximum in the
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, toprule=.15mm, rightrule=.15mm, colback=white, colbacktitle=quarto-callout-tip-color!10!white, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Box 1.1}, breakable, titlerule=0mm, colframe=quarto-callout-tip-color-frame, left=2mm, bottomtitle=1mm, toptitle=1mm, coltitle=black, arc=.35mm, bottomrule=.15mm, opacityback=0, opacitybacktitle=0.6]
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, toprule=.15mm, rightrule=.15mm, colback=white, colbacktitle=quarto-callout-note-color!10!white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Box 1.2}, breakable, titlerule=0mm, colframe=quarto-callout-note-color-frame, left=2mm, bottomtitle=1mm, toptitle=1mm, coltitle=black, arc=.35mm, bottomrule=.15mm, opacityback=0, opacitybacktitle=0.6]

Maynard Smith's (1978) model showing the cost of producing
males.\footnotemark{} Let \(N~asex~\) be the number of asexual females
at time 1, while \(N~sex~\) gives the total number of sexual individuals
(males plus females) at time 1. Let \(B~asex~\) give the number of
offspring produced by asexual females, and \(S~asex~\) gives the
survival probability of asexual offspring to maturity. The number of
surviving asexual offspring is then \(= B~asex~S~asex~\). Similarly, let
\(B~sex~\) be the number offspring produced by sexual females, and let
\(S~sex~\) give the survival probability of sexually produced offspring.
Maynard Smith assumed that all individuals reproduce once and then die.
Let \(r\) be the frequency of females in the sexual population. The
number of asexuals and sexuals at time 2 can then be calculated as is
the table below. (Note, we do not assume that the population is at
carrying capacity).

\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3300}}
  >{\centering\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3300}}
  >{\centering\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.3300}}@{}}
\toprule\noalign{}
...
\textbf{Number of sexuals} & \(N~sex~\) & \(rN~sex~(S~sex~B~sex~)\) \\
\textbf{Frequency of asexuals} & \(\frac{N~asex~}{N~asex~ + N~sex~}\) &
\(\frac{N~asex(S~asex~B~asex~)}{N~asex~(S~asex~B~asex~)+rN~sex~(S~sex~B~sex~)}\) \\
\end{longtable}
